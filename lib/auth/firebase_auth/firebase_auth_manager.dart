    ...
    try {
      ...
      if (e.code == 'requires-recent-login') {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content:
                  Text('Faça login novamente antes de excluir sua conta.')),
        );
      }
    }
  }
    ...
    try {
      ...
      if (e.code == 'requires-recent-login') {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text(
                  'Faça o login novamente antes de atualizar seu e-mail.')),
        );
      }
    }
  }
  ...
  Future resetPassword({
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text(
                'Não foi possível efetuar o login, verifique as informações e tente novamente!')),
      );
      return null;
    }
    ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<BaseAuthUser?> _signInOrCreateAccount(
    ...
    try {
      ...
      return userCredential == null
          ? null
          : ClinicalSoftwareV2FirebaseUser.fromUserCredential(userCredential);
    } on FirebaseAuthException catch (e) {
      final errorMsg = switch (e.code) {
        'email-already-in-use' => 'Este e-mail já esta em uso',
